...
export function EnsembleBadge({ content, data }: EnsembleBadgeProps) {
  ...
  return (
    <div className="flex gap-2 animate-slide-in">
      ...
      <div className="bg-purple-950/20 border border-purple-500/30 rounded-lg overflow-hidden text-xs max-w-[90%] w-full">
        <div
        ...
        {expanded && data && (
          <div className="p-3 bg-black/60 border-t border-purple-500/20 space-y-2">
            <div className="text-[10px] text-purple-300/80 uppercase font-mono font-semibold">
              Individual Responses Before Synthesis:
            </div>
            <div className="space-y-2">
              {data.individualResponses.map((item, idx) => (
                <div key={idx} className="p-2 rounded bg-purple-950/30 border border-purple-900/50 font-mono text-[11px]">
                  ...
                  <pre className="text-zinc-300 whitespace-pre-wrap max-h-32 overflow-y-auto">{item.response}</pre>
                </div>
              ))}
            </div>
          </div>
